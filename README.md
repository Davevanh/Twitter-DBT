...
<b>Intro to DBT</b><br>
DBT is a data transformation tool. It allows analysts to transform data that is already in a data warehouse using SQL statements that are triggered within the command line. A few of the benefits of DBT include: 
* Version control using Git(hub)
* DBT creates DAGs that determine the order in which models run
* DBT is modular: models are created once and they are referenced when they are re-used. This ensures that all analysts use the same code with shared definitions across the business
...
* staging: the models that are closest to the source data and will be referenced throughout the data transformation process
* marts: the final dim and fact tables that sit in the data warehouse

With the models set up, DBT creates the following DAG: 
